-- 문제1. 
-- 최고임금(salary)과 최저임금을 “최고임금, “최저임금”이란 타이틀로 함께 출력해 보세요.
...
-- 예) 2014년 07월 10일
select date_format(max(hire_date), '%Y년 %m월 %d일') as '마지막신입받은날'
	from employees;
...
select date_format(now(), '%Y') - date_format(max(birth_date), '%Y') as '최고어린사원',
